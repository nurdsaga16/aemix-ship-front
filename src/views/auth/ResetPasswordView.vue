...
const handleSubmit = async () => {
  ...
  if (!password.value.trim() || !confirmPassword.value.trim()) {
    error.value = 'Введите новый пароль и подтверждение.'
    return
  }
  ...
  try {
    await authStore.resetPassword(token.value, password.value.trim(), confirmPassword.value.trim())
    router.push('/login')
  } catch (e) {
    error.value = 'Не удалось сбросить пароль. Попробуйте ещё раз.'
  } finally {
        ...
        <div class="max-w-sm">
          <GlassCard class="mb-4" :delay="0.1">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h1 class="text-caps text-lg">НОВЫЙ ПАРОЛЬ</h1>
          ...
          <GlassCard :delay="0.2">
            <div>
              ...
              <div class="pt-2">
                ...
                >
                  <Save class="w-4 h-4" />
                  <span v-if="!isSubmitting">СОХРАНИТЬ ПАРОЛЬ</span>
                  <div
                    v-else
